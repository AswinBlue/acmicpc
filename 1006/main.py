...
def solve_DP(target):
    ...
    # 구역이 1개일 때가 모두 채워졌으면, 구역(열)을 하나씩 늘려가며 DP 채우기
    # 모든 case에 대해 다 고려가 되어야 하므로, else if 사용하지 않는다.
    for i in range(2, N):
        # (1) 위아래 모두 해결하는 case
        # 1. 위아래 모두 개별 부대 투입
        DP[i][0] = DP[i-1][0] + 2
        # 2. 위아래 모두 i-1 번째에 영역과 묶어서 처리
        if sector[0][i-1] + sector[0][i] <= W and sector[1][i-1] + sector[1][i] <= W:
            DP[i][0] = min(DP[i-2][0] + 2, DP[i][0])
        # 3. 위쪽만 i-1번째 영역과 묶어서, 아래쪽은 개별
        if sector[0][i-1] + sector[0][i] <= W:
            DP[i][0] = min(DP[i-1][2] + 2, DP[i][0])
        # 4. 아래쪽만 i-1번째 영역과 묶어서, 위쪽은 개별
...
if __name__ == '__main__':
    ...
    for t in range(T):
        ...
        #    DP[n][0]은 n번째 열 위아래를 모두 해결한 경우
        #    DP[n][1]은 n번째 열 위쪽만 해결하고, 아래쪽은 보류한 경우
        #    DP[n][2]는 n번째 열 아래쪽만 해결하고 위쪽은 보류한 경우를 의미한다.
        #    이 세가지 경우만 있으면 블럭을 놓을 수 있는 모든 경우에 대해 메모이제이션이 가능하게 된다.
        #    '해결' 이라는 것은 부대가 투입되었다는 의미이고, '보류' 라는 것은 아직 부대가 투입되지 않았다는 의미이다.
