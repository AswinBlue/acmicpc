...
while H:  # Dijkstra 알고리즘의 최대 반복 횟수는 V번이다. 
    ...
    current = heapq.heappop(H)[1]  # min heap을 사용해 계산시간 단축
    Print(current, visit, D, link[current], H)
    H = []  # pop을 해주고 나면 min heap을 초기화 (한 cycle을 돌면, 기존의 값들은 쓸모없어지기 때문)
