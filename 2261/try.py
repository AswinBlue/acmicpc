...
if __name__ == "__main__":
    ...
    promising = [save[0], save[1]] # push item to promising pool
    promising = sorted(promising, key=operator.itemgetter(1))  # y축에 대해 정렬
    ...
    while idx2 < unique_N:
        ...
        while idx < idx2:
            if abs(x - save[idx][0]) >= min_dist:
                ...
                search_result = binary_search(promising, 0, len(promising) - 1, save[idx])
                if search_result >= 0: # 이분탐색 결과를 promising에서 제거
                    promising.pop(search_result)
                    Print(f'Delete idx{search_result}, promising:{len(promising)}')
                idx += 1
            else:
        ...
        lower_bound = bisect_right(promising, y - min_dist, key=operator.itemgetter(1)) # find lower bound
        upper_bound = bisect_right(promising, y + min_dist, key=operator.itemgetter(1)) # find upper bound
        ...
        Print(f'promising:{(promising[0:10])}, idx:{idx}, idx2:{idx2}, minY:{y-min_dist}, maxY:{y+min_dist}')
        # check distance from all 'promising point' to 'target'
        for i in range(lower_bound, upper_bound):
            # check distance and update min_dist
            dist = distance_square(promising[i], target)
            powed_min_dist = min(powed_min_dist, dist)  # update powed_min_dist
            min_dist = sqrt(powed_min_dist)  # update min_dist
                

            Print(f'[{lower_bound}:{upper_bound}] i={i}, p1:{promising[i]}, p2:{target}, dist:{dist}/{powed_min_dist}')
            # Print(f'{promising}')
        # add 'target' to promising pool and increase index to find next target
        index_to_insert = bisect_right(promising, save[idx2][1], key=operator.itemgetter(1)) # find index to insert save[i] on 'promising' while not breaking 'y' axis arrangement
        promising.insert(index_to_insert, save[idx2])  # REF: https://stackoverflow.com/questions/44390818/how-to-insert-key-value-pair-into-dictionary-at-a-specified-position
        idx2 += 1
