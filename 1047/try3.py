...
def solve():
    ...
    for i in range(N):
        ...
        for j in range(i, N):
            x2 = x_sorted[j][X]
            y2 = x_sorted[j][Y]
            # ERROR: 두 개의 점을 사용하여 만든 영역은 모든 경우를 고려하지 않는다. 
            ...
            # 잘라낸 나무의 수 저장
            tree_to_chop = (idx_left_x) + (N - 1 - idx_right_x) + (idx_down_y) + (N - 1- idx_up_y)
            # 나무를 잘라서 획득한 목재
            lumber = 0
            # 울타리 설치에 필요한 목재
            ...
            # x 배열상 잘라내야하는 나무 체크
            for i in range(0, idx_left_x):
                if is_chopped[x_sorted[i][INDEX]] == 0:
                    # 이미 잘렸는지 체크 후 처리
                    is_chopped[x_sorted[i][INDEX]] = 1
                    lumber += x_sorted[i][VALUE]

            for i in range(idx_right_x + 1, N):
                if is_chopped[x_sorted[i][INDEX]] == 0:
                    # 이미 잘렸는지 체크 후 처리
                    is_chopped[x_sorted[i][INDEX]] = 1
                    lumber += x_sorted[i][VALUE]
            ...
            # y 배열상 잘라내야하는 나무 체크
            for i in range(0, idx_down_y):
                if is_chopped[y_sorted[i][INDEX]] == 0:
                    # 이미 잘렸는지 체크 후 처리
                    is_chopped[y_sorted[i][INDEX]] = 1
                    lumber += y_sorted[i][VALUE]

            for i in range(idx_up_y + 1, N):
                if is_chopped[y_sorted[i][INDEX]] == 0:
                    # 이미 잘렸는지 체크 후 처리
                    is_chopped[y_sorted[i][INDEX]] = 1
                    lumber += y_sorted[i][VALUE]
            ...
            for k in range(N):
                ...
                if is_chopped[v_sorted[k][INDEX]] == 0:
                    # 아직 자르지 않은 나무에 대해서만 처리
                    is_chopped[v_sorted[k][INDEX]] = 1
                    lumber += v_sorted[k][VALUE]
                    tree_to_chop += 1
